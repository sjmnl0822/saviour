...
local kickThread = task.spawn(lplr.Kick, lplr, 'Bedwars is no longer supported by Saviour V4, thank you for 5 years of support ❤️')
if coroutine.status(kickThread) ~= 'dead' then
	game:Shutdown()
end

local saviour = shared.saviour
local entitylib = saviour.Libraries.entity
local targetinfo = saviour.Libraries.targetinfo
local sessioninfo = saviour.Libraries.sessioninfo
local uipallet = saviour.Libraries.uipallet
local tween = saviour.Libraries.tween
local color = saviour.Libraries.color
local whitelist = saviour.Libraries.whitelist
local prediction = saviour.Libraries.prediction
local getfontbounds = saviour.Libraries.getfontbounds
local getvapeasset = saviour.Libraries.getvapeasset
...
local function addBlur(parent)
	...
	blur.Image = getvapeasset('newsaviour/assets/new/blur.png')
...
local function isFriend(plr, recolor)
	if saviour.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(saviour.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and saviour.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(saviour.Categories.Targets.ListEnabled, plr.Name) and true
end

local function notif(...) return
	saviour:CreateNotification(...)
end
...
run(function()
	...
	saviour:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
end)
entitylib.start()
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Saviour', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not saviour.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			saviour:Clean(connection:Connect(function(...)
				vapeEvents[event]:Fire(...)
			end))
		end)
	end

	saviour:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		saviour:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or saviour.Loaded == nil
			if saviour.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	saviour:Clean(vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	saviour:Clean(vapeEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	saviour:Clean(vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until saviour.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
